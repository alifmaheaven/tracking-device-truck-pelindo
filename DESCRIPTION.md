...
## 4. Newly Added Features (Keep Context)
   ...
   - **Interactive Route Waypoints & Label Kecepatan**: Saat fitur *History* menampilkan lintasan polyline, sistem akan melukis *circle markers (waypoints)* biru di seluruh lekukan perjalanan. Selain interaksi *hover tooltip* untuk mengintip persis jam kedatangan truk, sistem juga secara otonom membedah titik tengah (*midpoints*) antarsegmen rute untuk menaruh *Badge Lencana* kecepatan transparan (`km/j`). Anda dapat menyimak ringkasan kecepatan truk sepanjang jalan secara riil dan instan tanpa satu pun klik kursor.
   ...
   - **Dynamic Speed Line Chart & Smart Aggregation**: Modal histori rutin disokong oleh `Chart.js` pada blok khusus "Grafik Kecepatan". Grafik dilukis selaras warna ambang toleransi ngebut (hijau < 30km/j, kuning 30-50, merah > 50). Guna melawan semrawut titik data, saya memasukkan rekayasa *Bucket Aggregation*: sistem mengukur durasi total lintasan untuk memutuskan perlakuan *data downsampling*, merangkum kurva lewat pencarian titik kecepatan *Tertinggi (Max Point)* dalam wadah interval cerdas (1 Menit, 1 Jam, 8 Jam, hingga 1 Hari per satu titik kurva). Teks legendanya pun otomatis beralih rupa menyesuaikan durasi pengembaraan truk.
   - **Interactive HTML Custom Tooltip (Click-to-Route)**: Menggantikan fungsi bawaan Chart.js dengan *Custom HTML Tooltip* interaktif agar menerima dukungan klik DOM murni (`pointer-events: auto`). Di dalamnya tersemat tombol *"Lihat Rute"* canggih yang, saat ditekan, secara otomatis mengisolasi momen kejadian dengan presisi absolut. Fitur ini menarik timestamp grafik yang telah dikunci ketat murni pada detik pecahnya rekor *Peak/Max Speed* di dalam wadah intervalnya, lalu memicu API untuk melukis bentang waktu setipis `± 5 Menit` di sekitar titik kejadian. Peta yang dihasilkan menjadi luar biasa bersih dan *hyper-focused* meninjau spesifik manuver tersebut tanpa tercemar keruhnya riwayat lalu-lalang rute lainnya.
